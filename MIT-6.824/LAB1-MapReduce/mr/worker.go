...
func (w *worker) reqTask () Task {
	...
	if ok := call("Coordinator.GetOneTask", &args, &reply); !ok {
		panic("Fatal : worker GetOneTask Fail")
		os.Exit(1)
	}
	return *reply.Task
}
...
func (w *worker) run () {
	// if reqTask conn fails, worker exit
	for {
		t := w.reqTask()
		if !t.Alive {
			return
...
func (w *worker) reportTask(t Task, done bool, err error) {
	...
	if ok := call ("Coordinator.ReportTask", args, reply); !ok {
		...
		log.Fatal ("worker Report Task fail")
	}
}
...
func call(rpcname string, args interface{}, reply interface{}) bool {
	...
	// Make sure we close the socket every time (even in case of failure)
	defer c.Close()
